...
class RandomProjection(BaseEstimator, TransformerMixin, metaclass=ABCMeta):
    ...
    Any classes deriving from this at the very least should implement the following methods:
        _make_projection_components(X : torch.Tensor, Z : Optional[torch.Tensor]): this (void) method should initialize dependent variables (except n_features_)
        _project_features(X : torch.Tensor): this method should compute and return the projection applied to input data X
        _project_outer_prod(X : torch.Tensor, Z : torch.Tensor): this method should compute and return the projection applied to the array outer(X, Z)
    ...
    def _check_batch_dim(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None) -> None:
        ...
        Args:
            ...
            Z (Optional[torch.Tensor], optional): Second input data as a NumPy or CuPy array. Defaults to None.
    ...
    def _check_n_features(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None, reset : bool = False) -> None:
        ...
        Args:
            X (torch.Tensor): First input data as a NumPy or CuPy array.
            Z (Optional[torch.Tensor], optional): Second input data as a NumPy or CuPy array.
    ...
    def _validate_data(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None, reset : bool = False) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        Returns:
            Tuple[torch.Tensor, Optional[torch.Tensor]]: returns the input data(s) as a NumPy or CuPy array without any changes.
        """
        self._check_batch_dim(X, Z=Z)
        self._check_n_features(X, Z=Z, reset=reset)
        return X, Z

    def _check_n_components(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None) -> None:
    ...
    @abstractmethod
    def _make_projection_components(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None) -> None:
    ...
    def fit(self, X : torch.Tensor, y : Optional[torch.Tensor] = None, Z : Optional[torch.Tensor] = None) -> RandomProjection:
    ...
    def _project_features(self, X : torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Sketch of the input data, X, as a CuPy array.
        """
        pass

    @abstractmethod
    def _project_outer_prod(self, X : torch.Tensor, Z : torch.Tensor) -> torch.Tensor:
        """Abstract method for computing and returning the projection for input data outer(X, Z).

        Args:
            X (torch.Tensor): First input data as a CuPy array.
            Z (torch.Tensor): Second input data as a CuPy array.
        Returns:
            torch.Tensor: Sketch of the input data, outer(X, Z), as a CuPy array.
        """
        pass

    def _project(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None) -> torch.Tensor:
        """Wrapper method for computing and returning the projection for input data X or outer(X, Z).

        Args:
            X (torch.Tensor): First input data as a CuPy array.
            Z (Optional[torch.Tensor], optional): Second input data as a CuPy array.
        Returns:
            torch.Tensor: Sketch of the input data, X or outer(X, Z), as a CuPy array.
        """
        if Z is None:
    ...
    def transform(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None, return_on_gpu : bool = False) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Sketch of the input data, X or outer(X, Z), as a NumPy or CuPy array.
        """
    ...
    def __call__(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None, return_on_gpu : bool = False) -> torch.Tensor:
    ...
    def fit_transform(self, X : torch.Tensor, y : Optional[torch.Tensor] = None, Z : Optional[torch.Tensor] = None,
                      return_on_gpu : bool = False, **fit_params) -> torch.Tensor:
...
class GaussianRandomProjection(RandomProjection):
    ...
    def _make_projection_components(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None) -> None:
        ...
        self.scaling_ = 1. / torch.sqrt(self.n_components_)
...
class SubsamplingRandomProjection(RandomProjection):
    ...
    def _make_projection_components(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None) -> None:
        ...
        self.scaling_ = torch.sqrt(self.n_features_ / self.n_components_)
    ...
    def _project_features(self, X : torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Sketch of the input data, X, as a CuPy array.
        """
        return self.scaling_ * torch.take(X, self.sampled_idx_, axis=-1)
...
class VerySparseRandomProjection(RandomProjection):
    ...
    def _make_projection_components(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None) -> None:
        ...
        if self.sparsity == 'log':
            prob_nonzero = torch.log(self.n_features_) / self.n_features_
        elif self.sparsity == 'sqrt':
            prob_nonzero = 1. / torch.sqrt(self.n_features_)
        ...
        self.sampled_idx_ = torch.where(torch.any(utils.robust_nonzero(components_full), axis=0))[0]
        self.n_sampled_ = self.sampled_idx_.shape[0]
        self.components_ = torch.squeeze(torch.take(components_full, self.sampled_idx_, axis=1))
        self.scaling_ = torch.sqrt(1. / (prob_nonzero * self.n_components_))
    ...
    def _project_features(self, X : torch.Tensor) -> torch.Tensor:
        ...
        return self.scaling_ * utils.matrix_mult(
            torch.take(X, self.sampled_idx_, axis=-1).reshape((-1, self.n_sampled_)), self.components_, transpose_Y=True).reshape(X.shape[:-1] + (-1,))
    ...
    def _project_outer_prod(self, X : torch.Tensor, Z : torch.Tensor) -> torch.Tensor:
        ...
        X_proj, Z_proj = utils.subsample_outer_prod_comps(X, Z, self.sampled_idx_)
        return self.scaling_ * utils.matrix_mult(
            torch.reshape(X_proj * Z_proj, (-1, self.n_sampled_)), self.components_, transpose_Y=True).reshape(X_proj.shape[:-1] + (-1,))
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------
...
class CountSketchRandomProjection(RandomProjection):
    ...
    def _project_outer_prod(self, X_count_sketch : torch.Tensor, Z : torch.Tensor) -> torch.Tensor:
...
class TensorizedRandomProjection(RandomProjection):
    ...
    def _make_projection_components(self, X : torch.Tensor, Z : Optional[torch.Tensor] = None) -> None:
        ...
        self.scaling_ = 1. / torch.sqrt(self.n_components_) if Z is None else 1.
